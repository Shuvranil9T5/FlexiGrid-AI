...
def build_optimization_report(result: dict, source_label: str) -> bytes:
    ...
    for key, label in {"peak_kw":"Peak demand (kW)", "energy_cost_units":"Energy cost (units)", "solar_used_kwh":"Solar used (kWh)", "grid_energy_kwh":"Grid energy (kWh)"}.items():
    ...
    if result.get("scenarios"):
        story += [Spacer(1,10), Paragraph("Counterfactual scenarios", styles["Heading2"])]
        scenario_rows = [["Mode", "Status", "Peak kW", "Cost", "Solar kWh"]]
        for scenario in result["scenarios"]:
            scenario_rows.append([scenario.get("mode"), scenario.get("status"), scenario.get("peak_kw"), scenario.get("energy_cost_units"), scenario.get("solar_used_kwh")])
        scenario_table = Table(scenario_rows, colWidths=[35*mm,35*mm,28*mm,28*mm,28*mm])
        scenario_table.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#1e3a5f")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("GRID",(0,0),(-1,-1),.5,colors.grey),("PADDING",(0,0),(-1,-1),6)]))
        story.append(scenario_table)
    story += [Spacer(1,10), Paragraph(f"Mode: {result.get('mode','balanced').title()} | Solver: {result.get('solver_engine','constraint search')} | Status: {result.get('solver_status','unknown')} | Constraint violations: {result.get('constraint_violations',0)}", styles["BodyText"])]
