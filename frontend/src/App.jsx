...
import { downloadReport, loadDatasetSample, loadDemo, optimize, savePassport, uploadCsv } from "./services/api";
...
export default function App() {
  ...
  return <main>
    ...
    <section className="actions card"><button onClick={()=>run(loadDemo)} disabled={busy}>Load 7-Day Demo</button><button className="dataset" onClick={()=>run(()=>loadDatasetSample("uci"))} disabled={busy}>Load UCI Sample</button><button className="dataset" onClick={()=>run(()=>loadDatasetSample("iblend"))} disabled={busy}>Load I-BLEND Sample</button><label className="upload">Upload CSV<input type="file" accept=".csv" onChange={e=>e.target.files[0]&&run(()=>uploadCsv(e.target.files[0]))}/></label>{analysis&&<><select value={mode} onChange={e=>setMode(e.target.value)}><option value="balanced">Balanced</option><option value="cost">Lowest Cost</option><option value="peak">Peak Reducer</option><option value="carbon">Carbon-Aware</option></select><button className="secondary" onClick={runOptimizer}>Optimize Confirmed Loads</button></>}{busy&&<span>Processing…</span>}{error&&<span className="error">{error}</span>}</section>
